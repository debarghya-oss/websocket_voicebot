...
class SentenceChunkerForTTS:
    ...
    def __init__(self, sentences_per_chunk: int = 1, language: str = "en"):
        ...
        logger.debug(f"SentenceChunkerForTTS initialized with {sentences_per_chunk} sentence(s) per chunk for language '{language}' (optimized for 193 tokens/ms).")
    ...
    def chunk_text(self, full_text: str) -> Generator[str, None, None]:
        ...
        logger.debug(f"Segmented into {len(all_sentences)} sentences (yielding {self.sentences_per_chunk} per chunk for smooth streaming).")
