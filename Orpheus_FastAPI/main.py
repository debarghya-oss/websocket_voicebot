...
TTS_STREAM_MIN_GROUPS = 1  # Reduced from 5 - start streaming immediately
TTS_STREAM_SILENCE_MS = 0  # No silence padding - causes gaps
DEFAULT_MIN_DECODE_BATCH_GROUPS = 10 # Reduced from 7 for responsive decoding (fixes stuttering)
TTS_STREAM_PARTIAL_BATCH_TIMEOUT_MS = 100 # Reduced from 75ms - flush faster to prevent stuttering
        ...
        for i in range(num_groups_to_process):
            ...
            for j, t_id in enumerate(group):
                ...
                if layer_idx_from_id != expected_layer_for_pos:
                    ...
                    group_is_valid = False; break
                # --- ADDED: Validate code_idx is within valid SNAC range ---
...
def generate_speech_stream_bytes(
    ...
    last_code_received_time: Optional[float] = None  # Track when last codes arrived for timeout-based flushing
    ...
    try:
                ...
                try:
                    ...
                    if chunk_text:
                        ...
                        if new_codes:
                            ...
                            last_code_received_time = time.time()  # Track when codes arrived
                            #logger.debug(f"Parsed {len(new_codes)} new codes. Total accumulated: {len(accumulated_codes)}.") # Your original log

                            # Skip initial buffer stage - stream immediately on first code
                            ...
                            # Check for timeout-based partial batch flushing to prevent stuttering
                            if initial_buffer_processed and accumulated_codes and last_code_received_time is not None:
                                time_since_last_codes = (time.time() - last_code_received_time) * 1000  # ms
                                if time_since_last_codes > TTS_STREAM_PARTIAL_BATCH_TIMEOUT_MS or len(accumulated_codes) >= min_codes_required_per_batch:
                                    ...
                                    try:
                                        # Flush all available codes (or at least one group) to prevent silence gaps
                                        ...
                                        if codes_to_decode_count_batch > 0:
                                            ...
                                            if audio_chunk is not None and audio_chunk.size > 0:
                                                ...
                                                faded_chunk = apply_fade(audio_chunk, TARGET_SAMPLE_RATE, fade_ms=1)  # Minimal fade to reduce clicks
                                                audio_bytes_to_yield = faded_chunk.astype(np.float32).tobytes()
                                                yield audio_bytes_to_yield  # Stream immediately - no silence padding
                                                stream_successful = True
                                                last_code_received_time = time.time()  # Reset timeout after yield
                                            else:
        ...
        if len(accumulated_codes) >= codes_per_group: 
            ...
            if audio_chunk is not None and audio_chunk.size > 0:
                ...
                faded_chunk = apply_fade(audio_chunk, TARGET_SAMPLE_RATE, fade_ms=1)  # Minimal fade
                audio_bytes_to_yield = faded_chunk.astype(np.float32).tobytes()
                yield audio_bytes_to_yield  # Stream immediately - no silence padding
                stream_successful = True
            else:
