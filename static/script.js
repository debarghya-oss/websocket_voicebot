document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── RAG state ───────────────────────────────────────────────────
    // key: filename → { jobId, status, chunks, size }
    let ragFiles = {};
    ...
    function ragFileIcon(name) {
        ...
        return { pdf: '📄', txt: '📝', docx: '📄', doc: '📄' }[ext] ?? '📁';
    }

    function refreshUploadButtonState() {
        const hasDone = Object.values(ragFiles).some(f => f.status === 'done');
        UI.uploadBtn?.classList.toggle('has-file', hasDone || Object.keys(ragFiles).length > 0);
    }

    function upsertFileChip(filename, state) {
        if (!UI.fileChipRow) return;
        let chip = UI.fileChipRow.querySelector(`[data-rag-file="${filename}"]`);
        if (!chip) {
            chip = document.createElement('div');
            chip.className = 'file-chip';
            chip.dataset.ragFile = filename;
            UI.fileChipRow.appendChild(chip);
            UI.fileChipRow.classList.add('visible');
        }

        const { status, chunks = 0, size = 0 } = state;
        let statusColor, statusText;
        if (status === 'queued')     { statusColor = 'var(--text-muted)'; statusText = '⏳ Queued'; }
        else if (status === 'processing') { statusColor = 'var(--accent-dim)'; statusText = '⏳ Ingesting…'; }
        else if (status === 'done')  { statusColor = 'var(--accent)';     statusText = `✓ ${chunks} chunks`; }
        else                         { statusColor = 'var(--error)';      statusText = '✗ Failed'; }

        chip.innerHTML = `
            <span>${ragFileIcon(filename)}</span>
            <span class="file-chip-name" title="${filename}">${filename}</span>
            <span class="file-chip-size">${ragFormatSize(size)}</span>
            <span class="file-chip-status" style="color:${statusColor}">${statusText}</span>
            <button type="button" class="file-chip-remove" title="Remove">✕</button>
        `;
        chip.querySelector('.file-chip-remove').addEventListener('click', () => {
            delete ragFiles[filename];
            chip.remove();
            if (!UI.fileChipRow.querySelector('.file-chip')) {
                UI.fileChipRow.classList.remove('visible');
            }
    ...
    async function pollIngestJob(filename, jobId) {
        const INTERVAL = 1500, MAX = 80;
        let polls = 0;
        return new Promise(resolve => {
            const timer = setInterval(async () => {
                polls++;
                try {
                    const res  = await fetch(`/api/rag/ingest/status/${jobId}`);
                    ...
                    if (data.status === 'done' || data.status === 'failed' || polls >= MAX) {
                        clearInterval(timer);
                        refreshUploadButtonState();
                        resolve(data.status);
                    }
    ...
    async function ingestFile(file) {
        ...
        ragFiles[filename] = { status: 'queued', chunks: 0, size: file.size };
        upsertFileChip(filename, ragFiles[filename]);
        refreshUploadButtonState();
        try {
            const fd = new FormData();
            fd.append('file', file, filename);
            fd.append('topic', 'general');
    ...
    if (UI.uploadBtn && UI.fileInput) {
        ...
        UI.fileInput.addEventListener('change', () => {
            Array.from(UI.fileInput.files).forEach(file => {
                if (!ragFiles[file.name]) ingestFile(file);
            });
            UI.fileInput.value = '';
        });
    }
