...
async def websocket_conversation_endpoint(websocket: WebSocket):
    ...
    async def _process_pipeline(speech_audio: np.ndarray):
        ...
        try:
            ...
            # FIX: use system tempfile (no dir= argument)
            with tempfile.NamedTemporaryFile(delete=False, suffix=".wav") as tmp:
                sf.write(tmp.name, speech_audio, VAD_SAMPLE_RATE)
                tmp_path = tmp.name

            try:
                with open(tmp_path, "rb") as f:
                    audio_bytes = f.read()
                upload_file = UploadFile(filename="conversation_audio.wav", file=BytesIO(audio_bytes))
                if engine == "indic":
                    stt_result = await indic_transcribe(upload_file, language=language, decode_mode=decode_mode)
                else:
                    stt_result = await indic_transcribe(upload_file)
            ...
            # Flush backlogged audio from queue to prevent echo
            flushed = 0
            ...
            while offset < len(float32_samples):
                ...
                if is_processing:
                    ...
                    if speech_prob >= VAD_BARGEIN_THRESHOLD:
                        logger.info(f"[{session_id}] BARGE-IN (prob={speech_prob:.3f})")
                        interrupt_flag.set()
...
async def websocket_exotel_voicebot(websocket: WebSocket):
    ...
    async def send_exotel_media(
        ...
        for idx, chunk in enumerate(chunks):
            ...
            # 3200 bytes @ 8kHz PCM16 mono = 200ms
            outgoing_timestamp += 200
    ...
    async def _process_pipeline(
        ...
        try:
            ...
            # Flush backlogged audio from queue (echo prevention)
            flushed = 0
            while not audio_queue.empty():
                try:
                    audio_queue.get_nowait()
    ...
    try:
        ...
        while True:
            ...
            while offset < len(float32_samples):
                ...
                chunk = float32_samples[
                    offset : offset + VAD_CHUNK_SAMPLES
                ]
                ...
                utterance = vad_processor.process_chunk(
                    chunk
                )

                current_speaking = (
                    vad_processor.is_speech_active
                )
